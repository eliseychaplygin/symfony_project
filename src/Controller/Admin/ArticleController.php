...
class ArticleController extends AbstractController
{
    ...
    {
        $article = new Article();
        $article->setTitle('Есть ли жизнь после девятой жизни?')
...
EOF);
        ...
        if (rand(1, 10) > 4) {
            $article->setPublishedAt(new \DateTime(sprintf('-%d', rand(0, 50))));
        }

        $article->setAuthor('Барон Сосискин')
            ->setLikeCount(rand(0, 10))
            ->setImageFilename('cart1.jpg');
